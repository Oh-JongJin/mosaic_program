...
def mosaic(x, img, label=None, strength=1):
    if label:
        ...
        while True:
            try:
                ...
                strength -= 1
        mosaic_part = cv2.resize(roi, (w, h), interpolation=cv2.INTER_AREA)
        img[y:y + h, x:x + w] = mosaic_part


def blur(x, img, label=None, strength=1):
    # Plots one blur on img
    sigma = strength

    if label:
        x, y, w, h = int(x[0]), int(x[1]), int(x[2]) - int(x[0]), int(x[3]) - int(x[1])
        blur_it = img[y:y + h, x:x + w]
        blur_part = cv2.blur(blur_it, (sigma, sigma))
        img[y:y + h, x:x + w] = blur_part


def blur_circle(x, img, label=None, strength=1):
    sigma = strength
    ...
    if label:
        ...
        # img_all_blurred = cv2.GaussianBlur(img, (0, 0), sigma)
        img_all_blurred = cv2.blur(img, (sigma, sigma))
